...
from clients.client_utils import save_results, train_model, test_model, save_monitor_parameters
...
parser.add_argument('-alg', choices=['FedAvg', 'FedAdp', 'FedProx', 'Scaffold', 'FedDyn', 'FedDkw', 'FedNova', 'FedXGBllr'], default='Scaffold', help='FL algorithm', type=str)
...
##### monitor mode
monitor_metrics = {}
metr_names = ['train_time', 'overhead', 'fl_round', 'valid_time']
for name in metr_names:
    monitor_metrics[name] = []
    ...
    while True:
        if AWAKEN:          
            if not handler.training_end:# and not skip_train:
                ...
                monitor_metrics['train_time'].append(stop_train - start_train)
            ...
            monitor_metrics['fl_round'].append(stop_test - start_test)
            size_TX_model = sys.getsizeof(zlib.compress(pickle.dumps(detObj)))
            monitor_metrics['overhead'].append(size_TX_model)
        ...
        stop_test = time.time()
        monitor_metrics['valid_time'].append(stop_test - start_test)
        ...
        if running_loss <= target_loss or running_acc >= target_acc or handler.round >= rounds:
            ...
            save_monitor_parameters(monitor_metrics, id, run, algorithm.name, results_path)
