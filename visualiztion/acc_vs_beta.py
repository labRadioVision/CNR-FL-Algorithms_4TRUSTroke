...
def plot_data(file_param, metric):
    ...
    alg, alg_id = 'FedAvg', 0
    b_range = [0.001,0.1,0.3,1.0,10.0,100.0]
    for b in b_range:
        print(b)
        agg = []
            ...
            for d in range(devices):# FOR EACH DEVICE
                ...
                try:
                    mat = scipy.io.loadmat(
                        'Device_{}_samples_{}_batches_{}_size_{}_loc_{}_run_{}_dat_{}_gam_{}_alg_{}.mat'.format(str(d), file_param['samples'],file_param['batches'],
                                                                                                         file_param['size'],file_param['loc'],str(float(r)),file_param['data'], str(b), alg))
        ...
        #print(mean_agg - confidence_interval)
        plt.plot(X_sorted, mean_agg, label=b)

        plt.fill_between(X_sorted, mean_agg - confidence_interval, mean_agg + confidence_interval, alpha=0.2)

    plt.xlabel('Rounds')
    plt.ylabel(metric_dict[metric])
    plt.grid(True)
    plt.legend().set_title("Alpha")
    #    plt.savefig(metric_dict[metric]+'_Device_{}_samples_{}_batches_{}_size_{}_loc_{}_run_{}_dat_{}.png'.format(device, samples, batches,
    #                                                                                             size, loc, run, data))
    plt.show()
import os
os.chdir('D:/Program Files/PyCharm 2021.3.2/Projects/FL_platform/results/matlab/acc_vs_gamma_24_05')
